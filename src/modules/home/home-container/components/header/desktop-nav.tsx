        ...
        if (item.children) {
          return (
            <div key={item.label} className='relative h-full flex items-center group'>
              ...
              <div className='absolute top-full left-0 mt-2 hidden group-hover:block bg-white shadow-md border rounded-sm p-2 min-w-[180px] z-50'>
                {item.children.map(child => (
                  <Link key={child.label} href={child.href} className='block px-4 py-2 text-sm hover:bg-slate-50'>
                    {child.label}
                  </Link>
                ))}
              </div>
